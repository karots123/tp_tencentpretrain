...
def train_and_validate(args):
    ...
    # print('rank',args.rank,'mpu.get_data_parallel_group()',mpu.get_data_parallel_group())
        ...
        # Initialize with normal distribution.
            if args.deep_init:
                ...
                for n, p in list(model_for_training.named_parameters()):
                    if "gamma" not in n and "beta" not in n:
                        p.data.normal_(0, 0.02)
#        mmodel_for_trainingdel._megatron_batch_fn = get_batch_pipe
    
    #for model_module in model_for_training:
...
#            checkpoint_name, sd = model_for_training.load_checkpoint(args.pretrained_model_path,load_module_only=True,load_optimizer_states=False,load_lr_scheduler_states=False)
    else:
        if gpu_id is not None:
    ...
    if args.dist_train:
        ...
        train_loader = str2dataloader[args.data_processor](args, args.dataset_path, args.batch_size, dp_rank, args.world_size//(args.tensor_model_parallel_size*args.pipeline_model_parallel_size), gpu_id, True, model_for_dataloader)
    else:
