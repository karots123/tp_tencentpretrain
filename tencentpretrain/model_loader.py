...
def load_mp_model(model, model_path):
    ...
    pt_f = sorted([os.path.join(model_path, f) for f in prefix])
    ...
    if hasattr(model, "module"):
        model.module.load_state_dict(torch.load(pt_f[tp_rank], map_location="cpu")['module'], strict=False)
    else:
#        model.load_state_dict(torch.load(pt_f[tp_rank], map_location="cpu")['module'], strict=True)
        model.load_state_dict(torch.load(pt_f[tp_rank], map_location="cpu"), strict=True)
    return model
