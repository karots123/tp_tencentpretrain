...
from tencentpretrain.utils.global_vars import *
from tencentpretrain.initialize import *
import deepspeed
import sys
def run_test(local_rank):
        ...
        if mpu.get_data_parallel_rank() == 0:
            print(
                ...
                "model parallel rank ({}, {}): {}".format(
                    ...
                    sum(
                        [
                            sum([p.ds_numel if hasattr(p, "ds_id") else p.nelement() for p in model_for_training.parameters()])
                        ]
                    ),
                ),
                flush=True,
...
# 
def opt_init(args,model_for_training):
...
def train_and_validate(args):
    ...
    if args.dist_train:
        ...
        if not deepspeed:
           mp.spawn(worker, nprocs=args.ranks_num, args=(args.gpu_ranks, args, model_for_training, None,None,model_for_dataloader), daemon=False)
        else:
            worker(None, None,args, model_for_training,rank,gpu_id, model_for_dataloader)
...
def worker(ddp_proc_id, ddp_gpu_ranks, args, model_for_training, rank=None,gpu_id=None,model_for_dataloader=None):
    """
    Args:
    ...
    if not args.deepspeed:
        if args.dist_train:
            rank = ddp_gpu_ranks[ddp_proc_id]
            gpu_id =ddp_proc_id
    
    # Build optimizer.
    
    custom_optimizer,custom_scheduler,optimizer_grouped_parameters=opt_init(args,model_for_training)
    ...
    if args.dist_train:
        ...
        if args.use_mp:
           train_loader = str2dataloader[args.data_processor](args, args.dataset_path, args.batch_size, rank, args.world_size//(args.tensor_model_parallel_size*args.pipeline_model_parallel_size), gpu_id, True, model_for_dataloader)
        else:
...
class Trainer(object):
    ...
    def train(self, args, gpu_id, rank, loader, model, optimizer, scheduler):
        ...
        while True:
            # print('self.current_step',self.current_step,gpu_id)
            ...
            if args.deepspeed:
                if self.current_step % self.save_checkpoint_steps == 0:
                    if args.use_lora:
                        ...
                        sd = {}
                        sd['iteration'] = self.current_step
                        model.save_checkpoint(self.output_model_path, str(self.current_step), client_state = sd)
